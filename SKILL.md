...
- `-s` (save): Save outputs to `.zed/output/apex/`
...
| `-s` | `--save` | Save mode: output each step to `.zed/output/apex/` |
...
All outputs saved to PROJECT directory (where zed Code is running):
```
.zed/output/apex/{task-id}/
...
1. Locate the task folder in `.zed/output/apex/`
...
| `{save_mode}`           | boolean | Save outputs to .zed/output/apex/                   |
