...
save_mode: false # -s: Save outputs to .zed/output/apex/
...
{task_id} will be generated by the setup script:
  - Script auto-detects next available number in .zed/output/apex/
...
1. **Search for matching task:**
   ...
   ```bash
   ls .zed/output/apex/ | grep "^{resume_task}"
   ```
...
- Creates `.zed/output/apex/{task_id}/` directory
